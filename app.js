...
function foo(owner) {
    ...
    $.getJSON('//api.github.com/users/'+owner+'/repos',{},function(data){
        //console.log(owner);
        //console.log(data);
        let done = false;
        for(i of JSON.parse(JSON.stringify(data))) {
            ...
            let str = "<project-prototype name=\""+i.name+"\" desc=\""+i.description+"\" url=\""+i.html_url+"\" lang=\""+i.language+"\"";
...
function submit() {
    ...
    user.innerHTML = user.innerHTML.replace(user.innerHTML.split(" ")[user.innerHTML.split(" ").length-1], usrInp.value);
